    ...
    class PathFinder {
        constructor(startpos: {x: number, y: number}, endpos: {x: number, y: number}, sprite: Sprite, speed = 100) {
            ...
            for (let i = 0; i < difference_x + difference_y; i++) {
                if (!isWall(sprite.x + 5, sprite.y + 5, sprite.image) && endpos.y > sprite.y) {
                    sprite.setPosition(sprite.x, sprite.y + 1)
                    console.log(0)
                } else if (!isWall(sprite.x - 5, sprite.y - 5, sprite.image) && endpos.y < sprite.y) {
                    sprite.setPosition(sprite.x, sprite.y + 1)
                    console.log(1)
                } else if (!isWall(sprite.x + 5, sprite.y + 5, sprite.image) && endpos.x > sprite.x) {
